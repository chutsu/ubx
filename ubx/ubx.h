...
int uart_connect(uart_t *uart, char *port) {
  /* Connect */
  ...
  if (set_interface_attribs(uart->connfd, uart->speed) != 0) {
    return -1;
  }
...
int ublox_init(ublox_t *ublox, uart_t *uart) {
...
int ubx_write(const ublox_t *ublox,
  ...
  /* Transmit msg */
  size_t retval = uart_write(ublox->uart, frame, frame_size);
  if (retval != 0) {
...
int ublox_base_run(ublox_t *base, const int port, int *loop) {
  ...
  if (retval != 0) {
    LOG_ERROR("TCP Socket bind failed...");
    LOG_ERROR("%s", strerror(errno));
    return -1;
  } else {
    DEBUG("TCP Socket binded!");
  }
